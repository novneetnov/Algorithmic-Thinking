"""
Extra small tests for closest_pair_strip and fast_closest_pairs
Paste tests below at the bottom of your code
"""
import alg_cluster
import proj3_solution as student

# Test 1
pts = [[0.11, 0.75], [0.62, 0.86], [0.65, 0.68], [0.68, 0.48], [0.7, 0.9], [0.79, 0.18]]
cluster_list = [alg_cluster.Cluster(set([]), pts[idx][0], pts[idx][1], 1, 0) 
                        for idx in range(len(pts))]

print student.closest_pair_strip(cluster_list, 0.665, 0.182482875909) # should be (0.0894427191, 1, 4)
print student.fast_closest_pair(cluster_list)  # should be (0.0894427191, 1, 4)

print student.slow_closest_pair([alg_cluster.Cluster(set([]), 0, 0, 1, 0),
                               alg_cluster.Cluster(set([]), 1, 0, 1, 0)])


print student.closest_pair_strip([alg_cluster.Cluster(set([]), 0, 0, 1, 0),
                           alg_cluster.Cluster(set([]), 1, 0, 1, 0),
                           alg_cluster.Cluster(set([]), 2, 0, 1, 0),
                           alg_cluster.Cluster(set([]), 3, 0, 1, 0)], 1.5, 1.0)

print student.fast_closest_pair([alg_cluster.Cluster(set([]), 0, 0, 1, 0),
                         alg_cluster.Cluster(set([]), 1, 0, 1, 0)])
